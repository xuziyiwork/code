...
class Linear(Layer):
    ...
    def forward(self, input):
        '''Using your codes in Homework 1'''
        self.in_data = input
        self.out_data = np.dot(self.w, input) + self.b
        return self.out_data

    def backward(self, grad_output):
        '''Using your codes in Homework 1'''
        data_grad = np.dot(self.W, grad_output)
        self.grad_W = np.dot(grad_output, np.zeros((1, self.in_num)))
        self.grad_b = np.sum(grad_output, axis=1).reshape((-1,1))
        return data_grad
